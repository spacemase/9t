...
{ // prepare to process form data
  if ( !onlyEmail( $_POST['em'] ) ) { myRedirect( ADMIN_LOGIN . '?msg=3' ) ; }
  if ( !onlyAlphaNum( $_POST['pw'] ) ) { myRedirect( ADMIN_LOGIN . '?msg=3' ) ; }
  ...
  $selSQL = sprintf( "select AdminID,FirstName,Privilege,NumLogins from " . PREFIX . "Admin WHERE Email='%s' AND AdminPW=SHA('%s')",
                     $Email, $MyPass ) ;
  $result = @mysql_query( $selSQL, $myConn ) or die( trigger_error( mysql_error(), E_USER_ERROR ) ) ;
  if ( mysql_num_rows( $result ) > 0 )  # had a match
  { // valid user, create session vars, redirect!
    ...
    $NumLogins += 1 ;  # increment number of logins
